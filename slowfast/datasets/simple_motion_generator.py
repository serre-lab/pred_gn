...
icons = np.load('/users/azerroug/scratch/simple_motion/icons.npy', allow_pickle=True).item()
mnist = np.load('/users/azerroug/scratch/simple_motion/mnist.npy', allow_pickle=True).item()
...
shape_types ={
    'bars': None,
    'geometric': ['ellipse', 'rectangle', 'star', 'triangle'],
...
def generate_random_sequences(config, save_file=None):
    ...
    for i in range(n_sequences):
        sequence = {'n_frames': n_frames, 'size': size, 'number': n_objects}
        ...
        for i in range(n_objects):
            ...
            if 'translate' in s_cfg['motion']:
                ...
                positions = get_translation_sequence(shape_seq['motion']['position'], v_init, a, n_frames, boundary=shape_seq['boundary'])
...
def generate_motion_sequence(spec):
    s_cfg = spec
    n_objects = len(s_cfg['shapes'])
    for i in range(n_objects):
        # shape_seq={}
        shape_seq = s_cfg['shapes'][i]        
        # motion parameters
        if 'translate' in s_cfg['motion']:
            positions = get_translation_sequence(shape_seq['motion']['position'], shape_seq['motion']['translate']['v'], shape_seq['motion']['translate']['a'], s_cfg['n_frames'], boundary=shape_seq['boundary'])
            shape_seq['positions'] = positions

        if 'rotate' in s_cfg['motion']:
            angles = get_rotation_sequence(0, shape_seq['motion']['rotate']['v'], shape_seq['motion']['rotate']['a'], s_cfg['n_frames'])
            shape_seq['angles'] = angles

        if 'expand' in s_cfg['motion']:
            shape_seq['motion']['expand'] = {'v':v_init, 'a': a}
            sizes = get_scale_sequence(shape_seq['motion']['size'], shape_seq['motion']['expand']['v'], shape_seq['motion']['expand']['a'], s_cfg['n_frames'])
            shape_seq['sizes'] = sizes
            
        s_cfg['shapes'][i] = shape_seq  
