...
def random_short_side_scale_jitter_seg(images, min_size, max_size, masks=None):
    """
    Perform a spatial short scale jittering on the given images and
    corresponding boxes.
    Args:
    ...
    masks_ = torch.nn.functional.interpolate(
            masks,
            size=(new_height, new_width),
            mode="nearest",
        )
    while masks_ is None:
        masks_ = torch.nn.functional.interpolate(
            masks,
            size=(new_height, new_width),
            mode="nearest",
...
def random_crop_seg(images, size, masks=None):
    """
    Perform random spatial crop on the given images and corresponding boxes.
    Args:
        images (tensor): images to perform random crop. The dimension is
...
def horizontal_flip_seg(prob, images, masks=None):
...
def uniform_crop_seg(images, size, spatial_idx, masks=None):
